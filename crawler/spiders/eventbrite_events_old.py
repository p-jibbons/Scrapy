...
class eventbrite_events_old(Spider):
    name = "eventbrite_events_old"
    allowed_domains = []
    download_timeout = 30
    ...
    def start_requests(self):
        ...
        self.enddate = date.today() + timedelta(days = 10)
        self.city = "ca--san-diego"

        cursor = date.today()+ timedelta(days = 4)
        while cursor <= self.enddate:
            ...
            self.url = f"https://www.eventbrite.com/d/{city_name}/all-events/?start_date={date_short}&end_date={date_short}"
            print(self.url)
            print('\n' * 2)
            yield Request(self.url, callback=self.parse,meta={"date_short": date_short, "city_name": city_name, "city_id": city_id})
            time.sleep(6)
    ...
    def parse_listing(self, response):
        ...
        for e in jresp.get("events").get("results"):
            ...
            items['min_ticket_price'] = e.get("ticket_availability",  {}).get("minimum_ticket_price",  {}).get("major_value",{})
